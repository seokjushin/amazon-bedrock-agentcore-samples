# Amazon Bedrock AgentCore 런타임(Runtime)에서 Amazon Bedrock 모델과 LangGraph 에이전트(Agent) 호스팅

## 개요

이 튜토리얼에서는 Amazon Bedrock AgentCore 런타임(Runtime)을 사용하여 기존 에이전트(Agent)를 호스팅하는 방법을 배웁니다.

Amazon Bedrock 모델을 사용하는 LangGraph 예제에 초점을 맞춥니다. Amazon Bedrock 모델을 사용하는 Strands Agents는 [여기](../01-strands-with-bedrock-model)를 확인하고, OpenAI 모델을 사용하는 Strands Agents는 [여기](../03-strands-with-openai-model)를 확인하세요.
...
| 튜토리얼 유형       | 대화형                                                                       |
| 에이전트(Agent) 유형 | 단일                                                                        |
| 에이전트(Agent) 프레임워크(Framework) | LangGraph                                                    |
| LLM 모델            | Anthropic Claude Haiku 4.5                                                   |
| 튜토리얼 구성 요소   | AgentCore 런타임(Runtime)에 에이전트(Agent) 호스팅. LangGraph 및 Amazon Bedrock 모델 사용 |
| 튜토리얼 분야       | 범용                                                                         |
| 예제 복잡도         | 쉬움                                                                         |
